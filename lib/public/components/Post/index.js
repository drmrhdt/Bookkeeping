 ...
 * @param {tag} tag (DOM) to create an element
 * @param {properties} properties to create properties for a specific element
 * @param {text} text for an element
        ...
        ? h('.flex-wrap', { style: 'word-break: break-all' }, links.map((link, index) =>
            h('.flex-row', link[textKey].length < MAX_MENUITEM_TEXT_LENGTH ? [
                ...
                index + 1 < links.length
                    ...
                    : null,
            ] : truncatedMenuItem('a', {}, link[textKey]))))
        : 'None';
